...
int demo()
{
  ...
  REAL *row = NULL;
	...
	while(!infile.eof()){
		...
		sscanf(line, "%d %d", &dep_cond[k][0], &dep_cond[k][1]);
		k++;
	}

  /* We will build the model row by row
     So we start with creating a model with 0 rows and 2 columns */
  Ncol = nQueries * nQueries; /* there are two variables in the model */
  lp = make_lp(0, Ncol);
  if(lp == NULL)
  ...
  if(ret == 0) {
	...
	for(int i=0; i<nQueries*nQueries; i++){
		set_int(lp, i+1, TRUE);
		set_lowbo(lp, i+1, 0);
	}
