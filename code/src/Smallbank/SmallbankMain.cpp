...
void usage()
{
	cout << "Usage: SmallbankMain [options]" << endl << endl;
...
#ifdef DB_PGSQL
	SmallbankRunner smallbankRunner(szHost, szDBName, szDBPort, iListenPort,
			outputDirectory);
#else
	SmallbankRunner smallbankRunner(szDBName, szHost, szUser, szPass, szDBPort, szSocket,
									iListenPort, outputDirectory);
#endif
	try {
		smallbankRunner.startListener();
	} catch (CBaseErr *pErr) {
