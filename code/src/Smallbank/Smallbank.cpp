...
try {
		...
		PMsgDriverSmallbank pMessage = new TMsgDriverSmallbank;
		memset(pMessage, 0, sizeof(TMsgDriverSmallbank)); // zero the structure

		TMsgSmallbankDriver Reply; // return message
		INT32 iRet = 0; // transaction return code
		CDBConnection *pDBConnection = NULL;

		pDBConnection = new CDBConnection(
			pThrParam->pSmallbank,
			pThrParam->pSmallbank->mysql_dbname,
			pThrParam->pSmallbank->mysql_host,
			pThrParam->pSmallbank->mysql_user,
			pThrParam->pSmallbank->mysql_pass,
			pThrParam->pSmallbank->mysql_port_t,
			pThrParam->pSmallbank->mysql_socket_t);
#ifdef CAL_RESP_TIME
			timeval t1, t2;
			double exec_time;
		...
		CSmallbankdingDB smallbankdingDB(pDBConnection);
		...
		do {
			...
			try {
				//  Parse Txn type
					iRet = pThrParam->pSmallbank->RunSmallbankding(&pMessage->TxnInput, smallbankdingDB);
				txn_cnt++;
...
SmallbankRunner::SmallbankRunner(char *_mysql_dbname, char *_mysql_host, char * _mysql_user, char * _mysql_pass, char *_mysql_port, char * _mysql_socket, const int iListenPort, char *outputDirectory)
: m_iListenPort(iListenPort)
{
	...
	snprintf(filename, iMaxPath, "%s/SmallbankError.log",
			outputDirectory);
	char s[1000];
...
int SmallbankRunner::RunSmallbankding(TSmallbankdingTxnInput* pTxnInput, CSmallbankdingDB &smallbankding){
	TSmallbankdingTxnOutput smallbankOutput;

	smallbankding.DoSmallbankding(pTxnInput, &smallbankOutput);

	return smallbankOutput.status;
}

// Listener
void SmallbankRunner::startListener(void)
{
	int acc_socket;
...
// logErrorMessage
void SmallbankRunner::logErrorMessage(const string sErr, bool bScreen)
{
	m_LogLock.lock();
