...
, m_iTotalTrades((INT64)iHoursOfInitialTrades * SecondsPerHour * iLoadUnitSize / iScaleFactor)
...
, m_Security()
{
	...
	// Initialize total number of securities
	//
	...
	//
	m_iStartFromAccount = m_CustomerAccountTable.GetStartingCA_ID(iStartFromCustomer);
	...
	//
	m_iCurrentTradeId = (INT64)m_iHoursOfInitialTrades * SecondsPerHour 						
	...
	//
	m_BrokerTable.InitForGen(iLoadUnitSize, iStartFromCustomer);
	...
	m_Security.Init( 0, 0, NULL, NULL, m_fMeanInTheMoneySubmissionDelay );
}
...
bool CTradeGen::InitNextLoadUnit()
{
	...
	m_BrokerTable.InitForGen(m_iLoadUnitSize, m_iStartFromCustomer);
...
void CTradeGen::UpdateHoldings()
{
		...
		|| GetCurrentTradeType() == eLimitBuy)
	{
		...
		if (iNeededQty > 0)
		{
			...
			//
			NewHolding.iTradeId = GetCurrentTradeID();
			NewHolding.iTradeQty = iNeededQty;
			NewHolding.fTradePrice = GetCurrentTradePrice();
			NewHolding.BuyDTS = GetCurrentTradeCompletionTime();
			NewHolding.iSymbolIndex = GetCurrentSecurityIndex();
...
bool CTradeGen::GenerateNextHoldingSummaryRow()
{
	...
	if (m_iCurrentAccountForHoldingSummary < m_iLoadUnitAccountCount)
	{
		...
		m_SecurityFile->CreateSymbol(iSecurityFlatFileIndex, 
									...
									sizeof(m_HoldingSummaryRow.HS_S_SYMB));
...
void CTradeGen::GenerateHoldingHistoryRow(
							INT64 iHoldingTradeID, // trade id of the original trade
							INT64 iTradeTradeID,	// trade id of the modifying trade
...
*/
INT64 CTradeGen::GenerateNextTradeId()
{
...
void CTradeGen::GenerateNewTrade()
{
	...
	{	// A Market order
		...
		//
		m_NewTrade.fBidPrice = m_Security.CalculatePrice( m_NewTrade.iSymbolIndex, m_CurrentSimulatedTime );
	}
	else
	{	// a Limit Order => need to calculate the Submission time
		...
		m_NewTrade.SubmissionTime = m_Security.
	...
	//
	m_NewTrade.CompletionTime = m_Security.
...
void CTradeGen::GenerateCompletedTradeInfo()
{
	...
	m_CompletedTradeInfo.iCurrentBrokerId = 
...
void CTradeGen::GenerateTradeRow()
{
	...
	m_SecurityFile->CreateSymbol(GetCurrentSecurityIndex(), 
								...
								sizeof(m_TradeRow.m_Trade.T_S_SYMB));
...
void CTradeGen::GenerateCashTransactionRow()
{
	...
	if ( GetCurrentTradeIsCash() )
	{
		...
		m_SecurityTable.CreateName(GetCurrentSecurityIndex(), S_NAME, sizeof(S_NAME));
