...
public:
	...
	*/
	...
	{		
		...
		m_iWICount = m_rnd.RndIntRange(1, iMaxItemsInWL);	//number of items in the watch list
		...
		for (i=0, pos=m_set.begin(); pos!=m_set.end(); ++i, ++pos)
		{
			...
			m_SecurityFile->CreateSymbol( *pos, m_row.m_watch_items[i].WI_S_SYMB, sizeof(m_row.m_watch_items[i].WI_S_SYMB));
		}
