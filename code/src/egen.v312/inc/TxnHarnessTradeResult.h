...
class CTradeResult
{
	CTradeResultDB		m_db;
	int					m_ca_id;
	char 				m_tt_id[cTT_ID_len+1];
	char				m_symbol[cSYMBOL_len+1];
	int					m_trade_qty;
	char				m_exec_name[cEXEC_NAME_len+1];
	double				m_charge;
	int					m_c_id;
	int					m_b_id;
	int					m_tax_status;
	char				m_type_name[cTT_NAME_len+1];
	int					m_sell_type;
	int					m_market_type;
	double				m_price_purchase;
	double				m_value_trade;
	double				m_tax_rates;
	double				m_tax_amount;
	...
	double				m_cash_bal;
	int					m_d_c;
	char				m_cash_type[cSE_CASH_TYPE_len+1];	

public:
	CTradeResult(const char *szServer, const char *szDatabase)
