...
void usage()
{
	cout << "Usage: SeatsMain [options]" << endl << endl;
...
#ifdef DB_PGSQL
	SeatsRunner seatsRunner(szHost, szDBName, szDBPort, iListenPort,
			outputDirectory);
#else
	SeatsRunner seatsRunner(szDBName, szHost, szUser, szPass, szDBPort, szSocket,
									iListenPort, outputDirectory);
#endif
	try {
		seatsRunner.startListener();
	} catch (CBaseErr *pErr) {
