...
{
	m_pLogger->SendToLogger("DM object constructed.");
...
TIdent CDM::GenerateRandomCustomerId()
{
	return m_rnd.RndInt64Range(iDefaultStartFromCustomer, 
							 iDefaultStartFromCustomer + m_DriverGlobalSettings.cur.iActiveCustomerCount - 1);
}
...
void CDM::DoCleanupTxn( void )
{
	...
	m_CleanupTxnInput.start_trade_id = (INT64)(( m_DriverGlobalSettings.cur.iDaysOfInitialTrades * HoursPerWorkDay * SecondsPerHour * ( m_DriverGlobalSettings.cur.iActiveCustomerCount / m_DriverGlobalSettings.cur.iScaleFactor )) * 101 / 100 ) + 1;  // 1.01 to account for rollbacks, +1 to get first runtime trade
