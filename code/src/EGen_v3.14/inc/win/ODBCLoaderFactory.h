...
class CODBCLoaderFactory : public CBaseLoaderFactory
{
	...
	char		m_szLoaderParms[1024];		// optional parameters
	
public:	
	// Constructor
	CODBCLoaderFactory(char *szServer, char *szDatabase, char *szLoaderParms)
	{
		assert(szServer);
		assert(szDatabase);
		...
		strncpy(m_szLoaderParms, szLoaderParms, sizeof(m_szLoaderParms));
	}
