...
private:
	...
	bool SendToLoggerImpl(const char *szPrefix, const char *szTimestamp, const char *szMsg)
	{
		...
		if (!m_Log)
		{
			throw new CSystemErr(CSystemErr::eWriteFile, "CEGenLogger::SendToLoggerImpl");
		}
...
public:
	CEGenLogger(eDriverType drvType, UINT32 UniqueId, const char *szFilename, CLogFormatter* pLogFormatter)
	: CBaseLogger(drvType, UniqueId, pLogFormatter)
	{
		...
		if (!m_Log)
		{
			throw new CSystemErr(CSystemErr::eCreateFile, "CEGenLogger::CEGenLogger");
		}
	};
};
