 ...
 * possibly modification by either a customer or a broker to satisfy questions regarding 
 * a particular account, a group of trade transaction identifiers or a particular security.
 *
...
/*
 * Frame 1
 * responsible for retrieving information about the specified array of trade IDs and
	...
	WHILE i <= max_trades LOOP
		...
		IF num_updated < max_updates THEN
			...
			IF exch_name like '% X %' THEN
				SELECT	overlay(exch_name placing ' ' from position(' X ' in exch_name) for 3)
				INTO 	exch_name;
			ELSE
		...
		FOR rs IN 
			select num_found, num_updated, bid_price, exec_name, is_cash, is_market, trade_price, settlement_amount,
...
BEGIN
	...
	FOR rs IN SELECT T_BID_PRICE,
		...
		FOR aux IN
		SELECT  num_updated, rs.T_BID_PRICE::S_PRICE_T, rs.T_EXEC_NAME, rs.T_IS_CASH, rs.T_TRADE_PRICE::S_PRICE_T, rs.T_ID::TRADE_T,
...
BEGIN
	...
	FOR rs IN SELECT T_CA_ID,
		...
		FOR aux IN
		SELECT	0, rs.T_CA_ID, cash_transaction_amount, extract(year from cash_transaction_dts),
			extract(month from cash_transaction_dts), extract(day from cash_transaction_dts),
			extract(hour from cash_transaction_dts), extract(minute from cash_transaction_dts),
	...
	-- send num_updated
	FOR aux IN
	SELECT	num_updated, rs.T_CA_ID, cash_transaction_amount, extract(year from cash_transaction_dts),
		extract(month from cash_transaction_dts), extract(day from cash_transaction_dts),
		extract(hour from cash_transaction_dts), extract(minute from cash_transaction_dts),
		extract(second from cash_transaction_dts), cash_transaction_name, rs.T_EXEC_NAME,
		rs.T_IS_CASH, rs.T_TRADE_PRICE, rs.T_QTY, settlement_amount, 
